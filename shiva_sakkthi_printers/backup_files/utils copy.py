...
def generate_ewb(doc):
	...
	if isinstance(doc, six.string_types):
		doc = json.loads(doc)
	# payload = json.dumps(get_ewb_data("Sales Invoice", [doc['name']]))
	payload = json.dumps(
		{
	"supplyType": "O",
	"subSupplyType": "1",
	...
	frappe.log_error(title=_("E Invoice Request"), message=f'{headers} {payload}')
	frappe.errprint(f'{headers} {type(payload)}')
	response = requests.request("POST", url, headers=headers, data=payload)
	
	frappe.errprint(f'{response.text}')
	res = json.loads(response.text)
	frappe.db.set_value(
					"Sales Invoice",
					{"name": doc['name']},
					"ewaybill",
					res['result']["ewayBillNo"]
				)
	frappe.db.set_value(
					"Sales Invoice",
					{"name": doc['name']},
					"eway_bill_validity",
					res['result']["validUpto"]
				)
