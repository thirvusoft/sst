...
export default {
  ...
  methods: {
    ...
    customer_ledger(){
      ...
      if (!this.pos_profile.ts_is_stock_entry){
        this.items = [{ text: 'POS', icon: 'mdi-point-of-sale'}, { text: 'Customer Ledger', icon: 'mdi-account'}];
      }

      else{
        this.items = [{ text: 'Stock Entry', icon: 'mdi-point-of-sale'}];
      }
    },
    // End
    changePage(key) {
      // Customized By Thirvusoft
      // Start
      if (key == "POS" || key == "Stock Entry"){
        this.$emit('changePage', "POS");
      }
      if (key == "Customer Ledger"){
        evntBus.$emit('CustomerLedger');
      }
      // End
    },
    go_desk() {
